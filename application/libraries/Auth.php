...
class Auth
{
    ...
    */
    function is_logged_in($redirect = false, $default_redirect = true)
    {
        ...
        {
            if(!$this->menu_header) {
                $this->CI->db->select("admin_menu.*")->where('active', 1)->where('parent_id', 0)->where('group_id', $admin['group_id']);
                $this->CI->db->join("permissions", "permissions.menu_id = admin_menu.id");
                $parent_menus = $this->CI->db->order_by('priority', 'ASC')->get("admin_menu")->result();
                //pr($parent_menus, 1);
                foreach($parent_menus as &$menu){
                    $this->CI->db->select("admin_menu.*")->where('active', 1)->where('parent_id', $menu->id);
                    $menu->chilren_menu = $this->CI->db->order_by('priority', 'ASC')->get("admin_menu")->result();
                }
                $this->menu_header = $parent_menus;
            }
            return true;
        }
    }
